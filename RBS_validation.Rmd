...
# Filter (mi > 10) and print top modifications , sorted by descending mi
significant_mi <- mi %>% 
  filter(mi > 10) %>%
  arrange(desc(mi))
...
```{r}
# Perform the chi-square DiffTest to assess relative fitness against model 1:
...
```{r}
# Calculate Spearman's correlation between subscales:
...
```{r}
# Conduct reliability analysis for each factor and printing item statistics including the corrected-item total correlation
